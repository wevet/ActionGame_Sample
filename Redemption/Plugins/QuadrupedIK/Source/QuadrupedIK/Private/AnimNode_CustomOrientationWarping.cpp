...
FAnimNode_CustomOrientationWarping::FAnimNode_CustomOrientationWarping()
{
}

void FAnimNode_CustomOrientationWarping::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	FAnimNode_Base::Initialize_AnyThread(Context);
	BasePose.Initialize(Context);
	const FBoneContainer& BoneContainer = Context.AnimInstanceProxy->GetRequiredBones();
	IKFootRootBone.Initialize(BoneContainer);
	FBoneReference(PelvisBoneName).Initialize(BoneContainer);
	for (FBoneReference& Bone : SpineBones)
	{
		Bone.Initialize(BoneContainer);
	}
}

void FAnimNode_CustomOrientationWarping::CacheBones_AnyThread(const FAnimationCacheBonesContext& Context)
{
	BasePose.CacheBones(Context);
}

void FAnimNode_CustomOrientationWarping::Update_AnyThread(const FAnimationUpdateContext& Context)
{
	GetEvaluateGraphExposedInputs().Execute(Context);
	BasePose.Update(Context);
}

void FAnimNode_CustomOrientationWarping::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	if (!FMath::IsNearlyZero(LocomotionAngle, KINDA_SMALL_NUMBER))
	{
		...
		if (IKFootRootBone.IsValidToEvaluate(BoneContainer))
		{
			...
			const FQuat MeshToComponentQuat(FRotator(0.0f, 0.0f, 0.0f));
			...
			{
				if (!SpineBones[i].IsValidToEvaluate(BoneContainer))
					continue;

				FCompactPoseBoneIndex SpineBoneIndex = SpineBones[i].GetCompactPoseIndex(BoneContainer);
...
void FAnimNode_CustomOrientationWarping::GatherDebugData(FNodeDebugData& DebugData)
{
