...

{
	...
	if ((Owner->GetWorld()->IsGameWorld())) // && TickCounter > TickThrehold
	{
		...
		for (int32 Index = 0; Index < NumTransforms - 0; Index++)
		{
			...
			{
				const float SmoothValue = (1.0f - FMath::Exp(-SmoothFactor * CachedDeltaSeconds));
				if (bAtleastOneHit && bEnableSolver)
				{
					TotalSpineAlphaArray[Index] = FMath::FInterpTo(TotalSpineAlphaArray[Index], 1.0f, SmoothValue, FormatShiftSpeed);
				}
				else
				{
					TotalSpineAlphaArray[Index] = FMath::FInterpTo(TotalSpineAlphaArray[Index], 0.0f, SmoothValue, FormatShiftSpeed);
				}
				...
				if (!RestBoneTransformArray[Index].Transform.ContainsNaN() &&
					...
					(RestBoneTransformArray[Index].Transform.GetLocation() - UpdatedTransform.GetLocation()).Size() < RestBoneScaleThes * ComponentScale &&
					(AnimBoneTransformArray[Index].Transform.GetLocation() - UpdatedTransform.GetLocation()).Size() < AnimBoneScaleThes * ComponentScale)
				{
					const FQuat QuatDiff = (UpdatedTransform.Rotator().Quaternion() *
						RestBoneTransformArray[Index].Transform.Rotator().Quaternion().Inverse()).GetNormalized();
		...
		!Output.ContainsNaN())
	{
		...
		if (SpineAnimTransformPairArray.Num() > 0 && SpineFeetPair.Num() > 0)
		{
					...
					for (int32 JIndex = 0; JIndex < SpineAnimTransformPairArray[Index].AssociatedFootArray.Num(); JIndex++)
					{
						if (SpineFeetPair[Index].FeetArray.Num() > JIndex)
						{
							...
							SpineAnimTransformPairArray[Index].AssociatedFootArray[JIndex].AddToTranslation(
								ComponentToWorld.TransformVectorNoScale(SpineFeetPair[Index].FeetTraceOffsetArray[JIndex]));
...

			{
				...
				for (int32 Index = 0; Index < SpineFeetPair.Num(); Index++)
				{
					const FTransform BoneTraceTransform = Output.Pose.GetComponentSpaceTransform(
						SpineFeetPair[Index].SpineBoneRef.GetCompactPoseIndex(BoneContainer));

					const FVector& TracePosWS = ComponentToWorld.TransformPosition(BoneTraceTransform.GetLocation());
					...
					{
						//DrawDebugSphere(Owner->GetWorld(), BoneWS, 10.0f, 12, FColor::Cyan, false, 0.0f, 0, 1.0f);
					}
#endif
					...
					if (bCanRefresh)
					{
						...
						SpineLocationDiffArray[Index] = TracePosWS;
					}
					else
					{
			...
			checkSlow(!UQuadrupedIKLibrary::DoesContainsNaN(FinalBoneTransformArray));
			if (FinalBoneTransformArray.Num() > 0)
			{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
				if (CVarDebugFootIKPredictive.GetValueOnAnyThread() > 0)
				{
					// IK 補正後の Pelvis/Root の Component Space Transform を取得
					const FBoneTransform& PelvisIKOutput = FinalBoneTransformArray[0];
					const FTransform PelvisCorrectedCSTransform = PelvisIKOutput.Transform;
					const FVector PelvisCorrectedWSLocation = ComponentToWorld.TransformPosition(PelvisCorrectedCSTransform.GetLocation());
					//DrawDebugSphere(Owner->GetWorld(), PelvisCorrectedWSLocation, 15.0f, 12, FColor::Red, false, 0.0f, 0, 1.0f);
				}
#endif
...
void FAnimNode_CustomSpineSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	const FVector CharacterDirectionVector = ComponentToWorld.TransformVectorNoScale(CharacterDirectionVectorCS);
	...
	FormatShiftSpeed = ShiftSpeed; // (TickCounter < MAX_TICK_COUNTER) ? MaxSpeed : ShiftSpeed;
	MaxFormatedHeight = 0.0f; //PelvisHeightMultiplierCurve.GetRichCurve()->Eval(CharacterSpeed) * MaxDipHeight;
	MaxFormatedDipHeightChest = 0.0f; //ChestHeightMultiplierCurve.GetRichCurve()->Eval(CharacterSpeed)* MaxDipHeightChest;
	...
	if (IsValid(PredictionAnimInstance))
	{
		const float PredictiveZOffset = PredictionAnimInstance->GetPelvisFinalOffset();
		PelvisBaseOffset = PredictiveZOffset;
	}
	...
	if (SpineTransformPairArray.Num() > 0)
	{
		...
		if (bSpineSnakeBone)
		{
			if (SpinePointBetweenArray.Num() > 0 && SpineHitBetweenArray.Num() > 0 && SpineBetweenTransformArray.Num() > 0)
			{
				for (int32 Index = 0; Index < SpineHitBetweenArray.Num(); Index++)
				{
					...
					const FHitResult& CurHitResult = SpineHitBetweenArray[Index];

					if (CurHitResult.bBlockingHit)
					{
						if (SpinePointBetweenArray[Index].IsNearlyZero() || !bEnableSnakeInterp) //TickCounter < MAX_TICK_COUNTER || 
						{
							SpinePointBetweenArray[Index] = CurHitResult.ImpactPoint;
						}
						else
						{
							{
								...
								const FVector ImpactPointInverse = ComponentToWorld.InverseTransformPosition(CurHitResult.ImpactPoint);
							...
							SpinePointBetweenArray[Index] = FMath::VInterpTo(
								SpinePointBetweenArray[Index],
								CurHitResult.ImpactPoint,
		...
		for (int32 Index = 0; Index < SpineHitPairs.Num(); Index++)
		{
			if (SpineFeetPair.Num() > Index)
			{
				...
				{
					const TArray<FTransform>& B = SpineAnimTransformPairArray[Index].AssociatedFootArray;

					if (!B.IsValidIndex(JIndex))
					{
						continue;
					}
					...
					{
						if (SpineHitPairs[Index].FeetHitPointArray[JIndex].IsNearlyZero() || bIgnoreLerping) //TickCounter < MAX_TICK_COUNTER || 
						{
						...
						else
						{
							...
							const FVector ImpactPointInverse = ComponentToWorld.InverseTransformPosition(
							...
							SpineHitPairs[Index].FeetHitPointArray[JIndex] = FMath::VInterpTo(
					...
					else
					{
						...
						FeetZeroedPoint = ComponentToWorld.InverseTransformPosition(FeetZeroedPoint);
						FeetZeroedPoint.Z = 0.0f;
						FeetZeroedPoint = ComponentToWorld.TransformPosition(FeetZeroedPoint);
				...
				{
					if (SpineHitPairs[Index].ParentSpinePoint.IsNearlyZero() || bIgnoreLerping) //|| TickCounter < MAX_TICK_COUNTER
					{
					...
					else
					{
						if (bSpineSnakeBone)
						{
							...
							SpiralPoint = ComponentToWorld.InverseTransformPosition(SpiralPoint);
							const FVector ImpactPointInverse = ComponentToWorld.InverseTransformPosition(SpineHitPairs[Index].ParentSpineHit.ImpactPoint);
						...
						SpineHitPairs[Index].ParentSpinePoint = FMath::VInterpTo(
				...
				const FHitResult& LeftHit = SpineHitPairs[Index].ParentLeftHit;
				const FHitResult& RightHit = SpineHitPairs[Index].ParentRightHit;
				const FHitResult& FrontHit = SpineHitPairs[Index].ParentFrontHit;
				const FHitResult& BackHit = SpineHitPairs[Index].ParentBackHit;

				// left hit
				if (LeftHit.bBlockingHit)
				{
					if (SpineHitPairs[Index].ParentLeftPoint.IsNearlyZero())
					{
						SpineHitPairs[Index].ParentLeftPoint = LeftHit.ImpactPoint;
					}
					else
					{
						FVector SpiralPoint = SpineHitPairs[Index].ParentLeftPoint;
						SpiralPoint = ComponentToWorld.InverseTransformPosition(SpiralPoint);
						const FVector ImpactPointInverse = ComponentToWorld.InverseTransformPosition(LeftHit.ImpactPoint);
						...
						SpineHitPairs[Index].ParentLeftPoint = FMath::VInterpTo(
				...
				{
					if (SpineHitPairs[Index].ParentRightPoint.IsNearlyZero())
					{
						SpineHitPairs[Index].ParentRightPoint = RightHit.ImpactPoint;
					}
					else
					{
						FVector SpiralPoint = SpineHitPairs[Index].ParentRightPoint;
						SpiralPoint = ComponentToWorld.InverseTransformPosition(SpiralPoint);
						const FVector ImpactPointInverse = ComponentToWorld.InverseTransformPosition(RightHit.ImpactPoint);
						...
						SpineHitPairs[Index].ParentRightPoint = FMath::VInterpTo(
				...
				{
					if (SpineHitPairs[Index].ParentFrontPoint.IsNearlyZero())
					{
						SpineHitPairs[Index].ParentFrontPoint = FrontHit.ImpactPoint;
					}
					else
					{
						FVector SpiralPoint = SpineHitPairs[Index].ParentRightPoint;
						SpiralPoint = ComponentToWorld.InverseTransformPosition(SpiralPoint);
						const FVector ImpactPointInverse = ComponentToWorld.InverseTransformPosition(FrontHit.ImpactPoint);
						...
						SpineHitPairs[Index].ParentFrontPoint = FMath::VInterpTo(
							SpineHitPairs[Index].ParentFrontPoint,
							FrontHit.ImpactPoint,
							InterpSpeed,
							FormatTraceLerp);
					}
				}
				...
				{
					if (SpineHitPairs[Index].ParentBackPoint.IsNearlyZero())
					{
						SpineHitPairs[Index].ParentBackPoint = BackHit.ImpactPoint;
					}
					else
					{
						FVector SpiralPoint = SpineHitPairs[Index].ParentBackPoint;
						SpiralPoint = ComponentToWorld.InverseTransformPosition(SpiralPoint);
						const FVector ImpactPointInverse = ComponentToWorld.InverseTransformPosition(BackHit.ImpactPoint);
						...
						SpineHitPairs[Index].ParentBackPoint = FMath::VInterpTo(
...
void FAnimNode_CustomSpineSolver::UpdateInternal_Async(const FAnimationUpdateContext& Context)
{
...
void FAnimNode_CustomSpineSolver::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	if (SpineHitPairs.Num() > 0)
	{
		...
		FABRIK_BodySystem(
			Output,
			SpineFeetPair.Last().SpineBoneRef,
			Output.Pose,
			OutBoneTransforms);
...
void FAnimNode_CustomSpineSolver::LineTraceControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (!SpineHitPairs.IsEmpty())
	{
		for (int32 Index = 0; Index < SpineFeetPair.Num(); Index++)
		{
			const FCompactPoseBoneIndex ModifyBoneIndex_Local_I = SpineFeetPair[Index].SpineBoneRef.GetCompactPoseIndex(
			...
			for (int32 JIndex = 0; JIndex < SpineFeetPair[Index].FeetArray.Num(); JIndex++)
			{
				...
				SpineTransformPairArray[Index].AssociatedFootArray[JIndex].AddToTranslation(
					ComponentToWorld.TransformVectorNoScale(SpineFeetPair[Index].FeetTraceOffsetArray[JIndex]));

				const FCompactPoseBoneIndex ModifyBoneIndex_Knee_J = SpineFeetPair[Index].KneeArray[JIndex].GetCompactPoseIndex(
					Output.Pose.GetPose().GetBoneContainer());
...
void FAnimNode_CustomSpineSolver::InitializeBoneReferences(FBoneContainer& RequiredBones)
{
	...
	if (!bSolveShouldFail)
	{
		...
		for (int32 Index = 0; Index < SpineFeetPair.Num(); Index++)
		{
			...
			const int32 FeetNum = SpineFeetPair[Index].FeetArray.Num();
			SpineFeetPair[Index].FeetHeightArray.AddDefaulted(FeetNum);
	...
	const bool bDrawLine)
{
	...
	switch (RaycastTraceType)
	{
		...
		UKismetSystemLibrary::BoxTraceSingle(
			...
			StartLocation,
			EndLocation,
			FVector(1, 1, 0) * TraceRadiusValue * OwnerScale,
	...
	TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	float PredictiveZOffset = 0.f;
	if (IsValid(PredictionAnimInstance))
	{
		PredictiveZOffset = PredictionAnimInstance->GetPelvisFinalOffset();
	}

	FVector CurrentRootLoc = RootEffectorTransform.GetLocation();
	PredictiveZOffset *= ComponentScale;
	//CurrentRootLoc.Z += PredictiveZOffset;

	RootEffectorTransform.SetLocation(CurrentRootLoc);
}
	...
	FCSPose<FCompactPose>& MeshBases)
{
	...
	if (OriginPointIndex == 0)
	{
		...
		if (Owner)
		{
			...
			RightDirection = ComponentToWorld.TransformVectorNoScale(
	...
	else if (OriginPointIndex == SpineTransformPairArray.Num() - 1)
	{
		...
		if (Owner)
		{
			...
			// @TODO
			if (SpineHitPairs[OriginPointIndex].FeetHitArray.Num() > 2 || bUseFakeChestRotation ||
				SpineHitPairs[OriginPointIndex].FeetHitArray.Num() == 0)
			{
				if (SpineHitPairs[OriginPointIndex].ParentLeftHit.bBlockingHit && SpineHitPairs[OriginPointIndex].ParentRightHit.bBlockingHit)
				{
	...
	if (SpineHitPairs.Num() > PointIndex - 1)
	{
		...
		if (bAtleastOneHit && SpineHitPairs[PointIndex].ParentSpineHit.bBlockingHit)
		{
			...
			if (PointIndex == 0)
			{
				...
				RightDirection = ComponentToWorld.TransformVectorNoScale(LocalCross) * IntensifierSide;
...

			{
				...
				if (bAtleastOneHit)
				{
					...
					else
					{
						if (!bIgnoreLerping)
						{
							OutputTransform.SetLocation(FMath::VInterpTo(
